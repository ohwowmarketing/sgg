...
export default async (req, res) => {
  ...
  const cache = await db.collection('sdiofutures').findOne({
    _id: `${league}|${season}`,
    updated_at: {
  ...
  const url = `https://fly.sportsdata.io/v3/${league}/odds/json/BettingFuturesBySeason/${season}`
  ...
  const futures = markets.map((market) => ({
    ...
    sportsbooks: market.AvailableSportsbooks.map(
      (sportsbook) => sportsbook.Name
    ),
    bets: market.BettingOutcomes.filter((bet) => bet.IsAvailable).map(
      (bet) => ({
