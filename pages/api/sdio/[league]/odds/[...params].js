...
export default async (req, res) => {
  ...
  if (league === 'nfl') {
    ...
    url = `https://fly.sportsdata.io/v3/nfl/odds/json/GameOddsByWeek/${season}/${week}`
    key = process.env.SDIO_NFL
  } else {
    const [date] = params
    id = `${league}|${date}`
    timer = `${league} ${date} Odds`
    url = `https://fly.sportsdata.io/v3/${league}/odds/json/GameOddsByDate/${date}`
  ...
  const cache = await db.collection('sdioodds').findOne({
    _id: id,
    updated_at: {
